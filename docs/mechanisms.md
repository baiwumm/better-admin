...
- **边界**：池为纯内存，刷新后重建；页面滚动位置按产品约定不保活（每次切换回顶）；
  异常态（404 / loading / 校验失败 / 403）统一 overlay 主体区直显、URL 不变，池保持
  挂载不销毁——404 = routeTree 解析不到叶子组件（catch-all splat `/_authenticated/$`
  仅兜住 URL 匹配使布局稳定，刻意不注册组件以免污染该解析）；403 = 组件存在但不在
  用户菜单树；页面渲染异常由面板级 `PaneErrorBoundary` 接管（KeepAliveOutlet 旁路
  TanStack Match 渲染，路由器 errorComponent 够不到页面组件，冒泡到根会导致整布局
  消失；面板级边界仅当前面板显示 500、其余保活面板不受影响，重试 = 重置边界 + key
  递增重挂载子树）。根路由 notFound / error 直挂组件仅作布局外兜底（登录页等场景）。
  无权路径仍登记标签（点回仍 403），无匹配路由路径不登记标签。
