...
- **隔离机制**：测试连接串追加 `options=-c search_path=e2e` 启动参数，注入后**所有**连接（应用 `db/client.ts` 的 Pool、seed 子进程、DDL）无代码改动即整体落进 `e2e` schema；global-setup 一次性 DROP/CREATE schema → 直执行 `drizzle/*.sql` 建表 → `pnpm db:seed`，teardown 删 schema（`E2E_KEEP_SCHEMA=1` 保留调试）。**必须 5432 直连**：PgBouncer（6543 pooler）事务池不支持 startup parameters，search_path 会被静默丢弃。防御：建表后断言 `users` 表 `table_schema='e2e'`，不满足立即中止且不清理现场。
