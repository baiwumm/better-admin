...
**redirect 参数规则（用户拍板，对齐 Next 端 `proxy.ts` `buildSignInRedirect` 既有规则）**：`target === "/"` 跳 `/sign-in` **不带参数**（首页本就是登录后的默认落点），其余路径带 `redirect=<完整路径>`（encodeURIComponent）。nuxt 端路由守卫①（`auth.global.ts`）与 `redirectToSignIn` 两处同规则。**跨端现状（2026-09-22 逐端复核，替换原「React / Vue 仍为一律带参 / 一律裸跳旧形态」的过期备案）**：**路由守卫层四端已收敛**（React `routes/_authenticated/route.tsx:17` / Next `proxy.ts:36-46` / Vue `router/guards.ts:52-56` / Nuxt `middleware/auth.global.ts:50-54` 均为根路径裸跳、其余带参）；唯一残留分歧在 **api-client 的 401 硬跳层**——React `lib/api-client.ts:68`、Next `:41`、Vue `:83` 一律裸跳，Nuxt `app/lib/api-client.ts:91-97` 按本规则带参。四端消费侧都有 `isSafeRedirect` 校验，故裸跳不构成安全问题，只是回跳体验差异（该层触发时用户已登录态失效，多数场景仍在原页）。**已按方案 C 收敛（2026-09-22，用户拍板）**：Nuxt 的 `redirectToSignIn` 改为与其余三端同形的裸跳，但**保留**其独有的「已在 `/sign-in` 则早退」防御（别端缺这条，登录页收到意外 401 会整页重载）；同时把 `redirectingToSignIn = true` 移到该早退之后——原写法在此路径下（:80 置位 → :84 返回）会把标志位永久置真，之后真正的会话失效跳转反被自身挡掉。结论：**守卫层带参、api-client 层裸跳**是四端统一形态。
...
## 42. Nest e2e 的共享库 schema 隔离：search_path 连接串注入 + 迁移记录表/外键限定两处 drizzle 硬编码绕法（nest，2026-09-25）

- **隔离机制**：测试连接串追加 `options=-c search_path=e2e` 启动参数，注入后**所有**连接（应用 `db/client.ts` 的 Pool、seed 子进程、DDL）无代码改动即整体落进 `e2e` schema；global-setup 一次性 DROP/CREATE schema → 直执行 `drizzle/*.sql` 建表 → `pnpm db:seed`，teardown 删 schema（`E2E_KEEP_SCHEMA=1` 保留调试）。**必须 5432 直连**：PgBouncer（6543 pooler）事务池不支持 startup parameters，search_path 会被静默丢弃。防御：建表后断言 `users` 表 `table_schema='e2e'`，不满足立即中止且不清理现场。**Supavisor session pooler（pooler host 的 5432 端口）也不能用作 CI 测试库**：其对 `options` 启动参数的支持不稳定（实测连接偶发 `ECONNRESET` / `Connection terminated unexpectedly`，search_path 时有时无）——CI 的 nest-e2e job 改起本地 TLS postgres 容器（自签证书；应用 `db/client.ts` 硬编码 `ssl: { rejectUnauthorized: false }`，容器必须带 TLS），原生支持 startup parameters；本地开发继续用 Supabase 直连串。另注意 GitHub runner 无 IPv6 出站：免费层直连域名 `db.<ref>.supabase.co` 仅解析 IPv6，CI 上必然 `ENETUNREACH`。
- **坑 1——drizzle migrator 不能用**：`migrate()` 的迁移记录表硬编码在库级 `"drizzle"."__drizzle_migrations"`（**不受 search_path 影响**）。共享 Supabase 库的该表已有生产迁移记录，e2e 跑 `pnpm db:migrate` 会因「已应用」**静默跳过全部语句**且不报错。绕法：global-setup 按文件名字典序直接执行 `drizzle/*.sql`（全新 schema 无需增量记录，产物与 migrate 一致）。
- **坑 2——迁移 SQL 的外键带 `"public".` 限定**：drizzle-kit 生成的 SQL 里 `CREATE TABLE` 无 schema 限定（随 search_path 落进 e2e），但 `FOREIGN KEY ... REFERENCES "public"."xxx"` **显式限定**——共享库下会把 e2e 的外键悄悄挂到生产表（本例 e2e.user_roles 全部引用了 public.users）。绕法：执行前 `sql.replaceAll('"public".', '"e2e".')`。
- **坑 3——`pnpm db:seed` 全新库必炸（已修）**：seed 给两个一级目录菜单（系统管理/组织中心）写 `to: ''`，而 `menus_to_unique` 部分唯一索引 `WHERE "to" IS NOT NULL` **只豁免 NULL 不豁免空串**——两个 `''` 撞唯一冲突。生产库一直没炸是因为重跑时按 i18nKey 查回跳过插入、生产行本身是 NULL（2026-09-25 查证）。修复：seed.ts 两处 `to: ''` → `to: null`（重跑幂等路径零影响，新库/演示重置的数据形态与生产对齐）。
- **坑 4——refresh token JWT 无 jti，同秒重复登录 500（✅ 2026-09-25 已根治）**：`refresh_tokens.token_hash` 全局唯一，而 refresh token 是纯 JWT（payload `{sub, username, type:'access', ver}`、**无 jti**、`iat` 秒级精度）——同一用户**同一秒内两次登录**会签发出完全相同的 JWT 字符串，第二个 token 的哈希撞唯一索引 → 500 INTERNAL_ERROR。e2e 的 demo-login 连续快速登录同一池内用户稳定复现（CI 容器机器快，auth spec 也稳定命中）。**根治（用户拍板 jti 方案）**：两处签发点（`signTokens` 与 `refresh()` 轮换）的 refresh payload 加 `jti: nanoid()`——服务端不消费该值，仅保证每次签发的令牌串必然不同；存量无 jti 的令牌到过期自然淘汰，access token 不入库不受影响、不加。测试侧原有的轮间 sleep / login 节流规避已随之移除，demo spec 的无间隔连续登录保留为回归哨兵。
- **测试进程与业务解耦的手段**：`import 'express'` 在 pnpm 严格布局下解析不到（`@nestjs/platform-express` 的私有传递依赖，esbuild 也不擦除类型 import）——以 devDependency 补 `express@^5` 解决，不改业务源码。
