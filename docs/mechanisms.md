...
- **坑 4——refresh token JWT 无 jti，同秒重复登录 500（已发现，待拍板）**：`refresh_tokens.token_hash` 全局唯一，而 refresh token 是纯 JWT（payload `{sub, username, type:'access', ver}`、**无 jti**、`iat` 秒级精度）——同一用户**同一秒内两次登录**会签发出完全相同的 JWT 字符串，第二个 token 的哈希撞唯一索引 → 500 INTERNAL_ERROR。e2e 的 demo-login 连续快速登录同一池内用户稳定复现（全量跑必炸、单跑偶过）。测试侧以轮间 >1s 规避；**根治方向（需拍板）**：payload 加 `jti`（nanoid）或 refresh_tokens 唯一索引放宽为 `(user_id, token_hash)`。
