...
> Nuxt 端：M0（2026-09-12）工程基建与骨架 + 认证最小闭环——完成 6 项（认证 / 全站 i18n / 主题系统 / 错误页 / 异常页菜单 / 路由权限守卫），Dashboard 保持 ❌；`dev / build / lint / typecheck / test` 五绿 + 登录闭环与中英切换 GUI 冒烟通过。方案与里程碑见 `docs/nuxt-plan.md`（M1 服务端全量移植待启动）。
