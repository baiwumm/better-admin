...
| 命令面板              | ✅     | ✅       | ✅   | ❌    | Vue M0 接 UDashboardSearch；M3（2026-09-11）对齐 React 端 command-menu：菜单树按顶层分组拍平（多级显示「父级 › 页面」）+ searchText 键（祖先链/分组名可被搜索）+ 快捷链接组 + 自建主题组（走 design-theme-store 带揭示动画，关闭内置 colorMode 组）；Nuxt M0 已接 UDashboardSearch（菜单分组拍平 + 快捷链接 + 主题组走 colorMode，M4 接偏好后对齐揭示动画），完成状态随 M4 口径更新 |
