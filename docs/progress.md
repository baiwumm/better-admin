...
- **website 线上核验通过（用户确认）**：2026-09-25 三轮 website 改动（首页导航/对齐卡/技术栈卡、数据库设计页卡片网格）线上已生效，roadmap.mdx 保活表述修正随之落地；此前文档里「需随下次部署生效」的不确定态解除。
- **文档同步**：`feature-matrix.md` 三行现状表述收官（角色管理行 / Dashboard 行 / Playground 行行末各追加用户通过粗体句，行内历史叙述按「历史不回改」保留）；本条目为统一收官记录，此前各条目内的「GUI 走查待用户」均以本条为准。
...
- **功能对齐卡（stacks.tsx）**：4 行收敛为 3 行，"查看功能矩阵 →"文字链接并入"四端功能已全部对齐"行尾的 ↗ 图标（`aria-label="查看功能矩阵"` 保留）。
- **FAQ（faq.tsx）**：副标题只留"关于 Better Admin 的常见问题。"，GitHub 提问引导由手风琴下方既有行承担，不再重复。
- **技术栈卡片**：五张卡各加"↗ 在线演示"按钮（`STACKS` 数据新增 `url` / `actionLabel` 字段；页脚中途加过的四端链接行按用户要求移除，`DEMOS` 保持四端语义只服务文档站侧栏）；NestJS 卡按钮为"↗ 接口文档"，指向 `https://nest.baiwumm.com/docs`（Scalar 接口文档 UI，`SwaggerModule.setup('docs')` 不带全局 `api` 前缀，线上 200 已探测）。
- **验证**：浏览器实测桌面 1600 / 移动 390 两档视口逐项确认，`next build` 静态导出通过；形式经用户确认。
...
- **首页「技术栈与进度」状态纠正**：`components/landing/stacks.tsx` 五张卡原为 Vue「进行中」/ Nuxt「未启动」/ React「已完成」，全部改「已上线」（六端已于 2026-09-23 统一上线）并删掉随状态而生的 `active` 字段；29/29 分母按 `feature-matrix.md` 实数复核无误。
- **`/docs` 侧栏底部挂四端演示站外链**：`DocsLayout` 的 `links` 追加 React / Vue / Next.js / Nuxt 三枚 icon 型条目（图标复用 `StackGlyph`，地址收口到 `lib/site.ts` 新增的 `DEMOS`），绝对 URL 由 fumadocs 的 `Link` 自动带 `target="_blank"`；顺带把该区残留的 lucide 废弃 `Github` 图标换成项目自有 `GithubIcon`。
- **`content/**` 三处事实修正（按契约与代码证据）**：`architecture/api.mdx` 五个端点方法纠正（`POST /users/{id}/reset-password`、`PATCH /org/depts/sort`、`GET /org/posts/{id}/members` 去掉多写的 PUT、`POST /notifications/read-all`、`POST /notifications/{id}/read`，逐条对 `openapi.yaml:2779/4025/4444/5031/5050`）；`frontends/nuxt.mdx` 作废「Nuxt 是唯一锁 `packageManager` 的应用」（实测 react / vue / website 也锁 11.24.0，仅 next / nest 未锁）；`start/quick-start.mdx` + `start/repo.mdx` 的 `sync-versions` 分发清单由手写五端改为「按 `apps/` 实际子目录动态枚举（当前六个，含 website）」。审计同时核实**已对齐无需再查**的项：契约 v1.14.0 / 48 路径 / 77 操作、17 张表、六端域名与平台、各端与图表库版本、脚本与文档引用有效性（无失效引用、无「待部署」残留）。
- **仓库门面**：根 `README.md` 头部改为居中 Logo（`<picture>` 明暗双档，按 `ui-spec.md` §19.3 命名陷阱映射）+ 状态徽章行 + 技术栈徽章行 + emoji 链接行，五个二级标题加 emoji；新增根 `LICENSE`（MIT，`Copyright (c) 2026 baiwumm`，与 `vue3-okr-tree` 同文本）与根 `package.json` 的 `"license": "MIT"`（六端各自 package.json 未动，全 `private: true`）。徽章取证口径（中文标签须 percent-encode、六端状态用静态徽章而非 shields 实时探测、GitHub license 标识需 push 后才显示）见 `mechanisms.md` §41。
- **验证**：`tsc --noEmit` 干净、`pnpm build` 静态导出 20/20 页绿、无头 Chrome 实拍首页与 `/docs` 各一张（侧栏四枚外链图标与 `target="_blank"` 已在产物中核到）、12 条徽章 URL 逐条 curl 验 `<title>`。**运行时观感与暗色档待用户 GUI 走查**（`cd apps/website && node scripts/with-memory-cap.mjs dev -p 4319`）。
- **已知限制 / 待用户确认**：① 首页出现 beUI 原生 emerald 绿状态徽章，与 globals.css 原「不引入任何彩色」注释冲突——本轮按用户授权**改注释口径**（承认 beUI 原生状态色为唯一例外）而非退 neutral；② `ButtonLink` 无 `asChild`，内部 `/docs` 链接退化为整页导航（见 §40）；③ `components/theme-toggle.tsx` 未换 beUI `theme-toggle`（需重接 `theme-switch-animation` 的 ref 转发，已登记暂缓）；④ `content/index.mdx` 的「v1.5.0 起头像 Storage 豁免」版本号未能证实，**存疑待拍板**；⑤ 本轮改动**全部未提交**——工作区混有用户在途的 `/docs` 回退与 `docs/*` 编辑（`AGENTS.md` / `progress.md` / `mechanisms.md` / `globals.css` / `pnpm-lock.yaml` / 根 `README.md` 同文件并存），无法干净切分提交，提交方式等指令；⑥ `git tag v0.2.0` 仍按原决定暂缓。

### 文档收尾二轮：website 改造范围收窄 + docs 目录处置 + Release 自动化（2026-09-23）

- **website beUI 改造范围收窄（用户拍板，修正上一条目的改造口径）**：beUI 仅保留**首页**（landing：按钮 / 卡片 / 顶部导航 / Badge / Questions 手风琴），**`/docs` 文档区回退 fumadocs-ui**——恢复 RootProvider（含静态搜索 `type:'static'`）与 `lib/i18n.ts`、`mdx-components.tsx`、`app/docs/**`、`stack-tabs.tsx`、globals.css（含 `--color-fd-*` 黑白 token 覆盖段——两区视觉不割裂的关键）、`fumadocs-ui` 依赖与 lockfile；删除 12 个自绘文档组件，自绘 accordion 迁至 `components/landing/`（首页 Questions 继续用）；保留 QuickStartFlow 移动端横滑修复（去掉已删的 `docs-scrollbar` class）。验证：frozen install 一致 + 静态导出 20/20 页构建绿 + 首页 / 文档页 / Questions / 暗色截图走查通过。mechanisms §37 已补范围修正注记。
- **docs/ 目录处置（按上一条目建议清单执行）**：删除 4 份已完成阶段的计划文档（`vue-plan.md` / `nuxt-plan.md` / `plan-phase0-execution.md` / `plan-dashboard-playground.md`，关键结论已沉淀 progress / mechanisms / launch-runbook，git 历史可随时找回）；`launch-audit.md` 加**已归档**标头（只读追溯，残余登记项由 code-review-backlog 与本文件承接）；AGENTS §13 文档表同步删行、§17 ④ / §18-8 / §19 的 4 处死链改为「原文档已删除 + 指向 progress/runbook」注记；requirements §Dashboard 注 / ui-spec §playground 行指针改指 progress.md。progress.md 旧条目内的历史引用按「历史不回改」保留。
- **新增 `.github/workflows/release.yml`（tag 推送自动 Release）**：骨架取自 theme-switch-animation 仓同名 workflow，按产品仓形态裁剪——去掉 npm 发包 / changesets / 人工审批链，保留「tag = 根 package.json version」与「tag 落在 origin/main」两道校验；正文无 CHANGELOG 可取，改为 git log（上一 tag → 本 tag）按 Conventional Commits 分组生成（组内同主题去重 + compare 链接），Release 已存在时更新正文而非报错。本地实测（v0.1.0 tag 冒烟）：区间 74 提交、分组与中文主题正确。
- **状态**：以上改动均**未提交**（等用户指令）；website 需重新部署 CF Workers 后走查。
...
- **已知限制 / 未闭环**：① **#45 挂 CI** 未做——新建 workflow 会立即消耗 Actions 额度并可能对现有分支报红，属影响共享系统的变更，待批准；② ~~#9 super_admin 授权缺口~~ **已补**（用户批准后由 `migrate-menus-add-super-admin-grants.ts` 回填 4 条；实测构成与原记录不同，见该文件与 `launch-audit.md` #9）；③ **#47 另一半**（`/settings` 直连四端行为不一致：占位页 / 空 div / 404）待统一形态；④ **#51 文档站 `theme-switch-animation` 仍 0.1.0**（升级要动供应链白名单）；⑤ **#56 `usedIn` 承诺未兑现**（四端 meta 仍是 TODO、全仓无赋值）；⑥ 全部 GUI 走查、双端契约冒烟、Next/Nuxt 补跑 build、`git tag v0.2.0`、Shadcn 幽灵条款（#18）等**需拍板项**见台账。
...
- **⚠️ 上线前必做（人工清单）**：超管 `admin` 当前仍可用种子默认密码 `admin123` 登录（curl 实测），公开上线前**必须改密**；`super_admin` 角色 role_menus 仅 24/28（缺 exception 三页 + 主题切换动画页，重置前即如此，超管靠 -1n 聚合免检不受影响）。（**2026-09-22 更新**：密码已由用户改毕，实测 `admin/admin123` 返回 401 `INVALID_CREDENTIALS`；授权缺口按实测为 30/26、缺异常页目录 + 三子页共 4 条，已幂等回填并交叉复核 gap=0。）
