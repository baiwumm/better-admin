...
> **状态：M0 + M1 已完成（2026-09-12/13）**——M0 工程基建与骨架 + 认证最小闭环；M1 服务端全量移植（70 个方法端点 / 11 个业务 service / 契约冒烟脚本 `scripts/contract-diff.mjs`）。**M2（核心系统管理六模块）待开工指令**。各里程碑细节见 `progress.md` 对应条目。
