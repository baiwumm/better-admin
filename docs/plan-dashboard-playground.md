...
- **头像**：脚本在**服务端下载真人风格头像后转存 Supabase Storage `avatars` bucket**（脚本持密钥、浏览器不接触，符合 `AGENTS.md` §5 豁免规则），头像 URL 落库为自家 Storage 域名，无外链依赖。来源选逼真的真人照片风格（faker `image.personPortrait({ sex })` 或同等真人风格数据集），**性别与用户性别字段匹配**；单张下载失败回退空头像走首字。**Storage 文件名用确定性命名**（如 `demo/0012.jpg`，按 seed 序号），重置时同名覆盖——用户主键是 nanoid 每次重置都会变，文件名若跟主键走会在 Storage 累积垃圾文件。**面孔口径（2026-09-17 拍板）**：faker `personPortrait` 数据集为 AI 生成的多族裔面孔、无法按族裔筛选，且无可靠免费的中国面孔真人风格公开数据源，**保持现状**；姓名 / 用户名 / 邮箱已中国化（拼音账号 + `@better-admin.com` + 分性别名字池）。
...
- 登录成功 toast 沿用系统既有行为，文案为「登录成功，欢迎 {姓名}」，**不带角色**（2026-09-17 拍板：`AuthUser.roles` 为角色 code，对访客无意义；展示角色名需契约追加字段，不为此扩契约）。
...
```text
┌──────────────────────────────────────────────────────────────┐
│  欢迎横幅：问候 + 副标题 + 日期/天气 Chip  [快捷入口按钮组]     │
├──────────────────────────────────────────────────────────────┤
│  KPI 行（4 张统计卡，等宽网格）                                │
│  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐             │
│  │图标徽标  │ │ 图标徽标 │ │ 图标徽标 │ │ 图标徽标 │             │
...
4. **欢迎横幅（React 基准 2026-09-19）**：主色径向光晕 + 右上同心圆环装饰的 Card（token 取色 `color-mix`，深色模式透明度微增）；左侧时段问候（`text-2xl`）+ 情绪副标题 + 信息 Chip 行（当日日期为真实 `Intl.DateTimeFormat` 本地时间；天气为按日期确定性生成的虚拟演示数据，Chip `title` 标注「演示数据」）；右侧快捷入口按钮（用户管理 primary / 公告管理、日志管理 outline，导航语义），**按用户可见菜单过滤**（与路由守卫同判据），无可见菜单时整组不渲染。
5. **加载态**：整页 Skeleton 骨架（布局与真实内容一致，杜绝跳变）；图表区骨架为等高灰色块。
6. **深浅色模式**：两套模式均需逐像素过一遍；深色下可给主图表区极弱的主色光晕（主色 5%~8% 透明度径向渐变），克制使用，不新增任何色值。
7. **硬性约束**：全部使用现有项目级 Design Tokens（`apps/react/src/styles/theme.css` oklch 变量、radius 10px、既有 shadow / spacing），**禁止新增颜色 / 圆角 / 阴影 / 字体**（`AGENTS.md` §7.3）；Vue 端不移植 token，用 Nuxt UI 默认体系实现同等视觉（`AGENTS.md` §21）。
8. **逼真数据口径**：数字全部来自真实接口聚合（§4.3），不用假数据硬编码；接口未就绪前前端以 Skeleton 呈现。数据底座即 Phase 0 的 faker 数据集（欢迎横幅的天气为用户明确允许的虚拟演示数据，属唯一例外，已作标注）。
