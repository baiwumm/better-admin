...
- **图例用 `VisBulletLegend`**（库的内置通用图例，官方 Donut 示例就是它，不是地图专用）：`items` 形如 `{ name, color?, shape?, inactive?, hidden? }`，标签由 d3 `.text(d => d.name)` 渲染（无 innerHTML 注入面）；**item 没有 value / extra 字段**，要显示数值只能并进 `name` 文本。样式变量 `--vis-legend-label-color` / `--vis-legend-label-font-size` / `--vis-legend-bullet-size` / `--vis-legend-item-spacing`；根节点是块级 flex（内容左对齐），居中要靠外层 `flex justify-center` + 自身 `w-fit`。
- **已确认的缺口**（选型时接受）：没有 Radar / Funnel / Gauge / 日历热力图（`VisHeatmap` 是矩阵热力，不是 GitHub 贡献图）；`VisArea` 无 `gradient` 属性。
