...
| `/playground/*` | Playground 演示场 | Playground | 演示模式（`PlaygroundIntro` 信息卡 + 组件演示区，10 页含三级菜单；详见 progress.md 2026-09-14 ~ 09-16 条目） | ✅ 已实现（四端） |
...
## 19. 品牌标识资源（Logo / Favicon）

> **单一真源**：`assets/logo/`。本节是该资源的登记与规则单源（几何常量、命名对照、验收口径）。生成物**不得手工编辑**——改几何请改 SVG 源，再重跑生成脚本（§19.6）。

### 19.1 定稿方案

**方案 03「Monogram B」精修版** —— 深底方章 + 字母 B。字母 B 由「一根主干 + 上下两碗 + 两个字腔」构成，取项目名 **B**etter 的首字母。

| 项 | 值 |
| --- | --- |
| 画布 | `256 × 256`（`viewBox="0 0 256 256"`） |
| 方章外圆角 | `rx = 57`（≈ 22.3%） |
| 方章填充 | 主版本 `currentColor`；静态变体 `#0A0A0A` / `#FFFFFF` |
| 字母 B 圆角 | `rx = 16`（图标几何 `rx = 20`） |
| 字腔圆角 | `rx = 14`（图标几何 `rx = 16`） |
| 图形极性 | 只有两级：方章（前景）/ 字母 B（背景或字块）。**无第三级灰阶、无透明度** |

### 19.2 几何常量

**品牌几何**（`logo.svg` / `logo-dark.svg` / `logo-light.svg`；元素安全框 `60 – 196`）：

| 元素 | x | y | width | height | rx |
| --- | --- | --- | --- | --- | --- |
| 竖柱（主干） | 64 | 64 | 52 | 128 | 16 |
| 上碗 | 112 | 64 | 80 | 52 | 16 |
| 下碗 | 112 | 140 | 80 | 52 | 16 |
| 上字腔（挖空） | 136 | 74 | 36 | 32 | 14 |
| 下字腔（挖空） | 136 | 150 | 36 | 32 | 14 |

- 图形 bbox = `64 – 192`（`128 × 128` 居中），四周留白 `64`。
- 两碗垂直间距 `24`（`64 + 52 = 116` → `140`）；主干底部与下碗底部齐平于 `192`。
- 主干右边界 `116` 与碗左边界 `112` 横向咬合 `4`（防止极限尺寸下主干与碗脱离）。

**图标几何**（`favicon.svg` / `favicon-light.svg`；元素安全框 `52 – 204`）：
...
| 下碗 | 112 | 144 | 92 | 60 | 20 |
| 上字腔 | 136 | 64 | 48 | 36 | 16 |
| 下字腔 | 136 | 156 | 48 | 36 | 16 |

- 图形 bbox = `52 – 204`（`152 × 152`），占画布 59.4% —— 比品牌几何更满，保证 16px 下 B 仍可读：16px 时竖柱 `4px`、两碗间距 `2px`、字腔 `3 × 2.25px`；20px 时依次 `5px` / `2.5px` / `3.75 × 2.81px`。
- 圆角后墨迹最远点距画布中心 `99.2` < PWA maskable 安全圆半径 `102.4`，可安全用作 maskable 图标。
- **为何两档**：品牌几何克制（字标 / ogimg / 大尺寸），图标几何饱满（浏览器标签 16 / 32）。二者是同一套比例规则的两档留白，**不是两套图形语言**。

### 19.3 资源清单与命名对照

⚠️ **命名陷阱**：`assets/logo/` 按「图形自身外观」命名，`apps/*/public/` 按「用于哪套主题」命名——**两者语义相反**。这是刻意对齐仓库既有资产名（现有代码 `import logo from "/logo.svg"`），装配时必须按下表映射，不要「顺手统一」。

| `assets/logo/`（真源，按外观） | 外观 | 装配到 `apps/*/public/`（按主题） |
| --- | --- | --- |
| `logo.svg` | `currentColor` 方章 + 挖空 B | —（不装配；供内联 SVG / ogimg 自动反色） |
| `logo-dark.svg` | 黑底白块 | `logo.svg`（浅色主题用） |
| `logo-light.svg` | 白底黑块 | `logo-dark.svg`（深色主题用） |
| `favicon.svg` | 图标几何，`prefers-color-scheme` 双态 | `favicon.svg` |
| `favicon-light.svg` | 图标几何，白底黑块单态 | `favicon_light.svg`（仅 react / next） |
| `favicon.ico` | 16 / 32 / 48 三尺寸 | `favicon.ico` |
| `logo.png` / `logo-dark.png` | 256 位图 | 同名 |
| `preview.png` | 预览图（设计验收用） | — |
...
2. **单一真源 + 单一几何**：亮暗两版必须是**同一份几何、只换填充色**，禁止为视觉对齐单独调整某一版的坐标或线宽。主版本 `logo.svg` 用 `currentColor` + `<mask>` 挖空实现自动反色，**禁止为反色维护第二份几何定义**。
3. **亮暗等价**：两版块面积 / 圆角 / 间距逐像素一致（口径见 §19.5）。
4. **极限底色自洽**：纯黑底取白、纯白底取黑，块面积不变（ogimg 黑白风格要求，AGENTS §19）。
5. **不得绕过生成脚本**：`favicon.ico` 与派生 PNG 全部由脚本产出，禁止手工替换或只改某一端。
6. **几何变更必须重新验收**：改动 §19.2 任何数值后，重跑脚本并确认 §19.5 各项通过。

### 19.5 验收结果（2026-09-14 实测）

| 检查项 | 结果 |
| --- | --- |
| 亮暗两版一致性（品牌几何 128px，逐像素角色比对） | 比对 `16354` 像素，**不一致 `0`**；Alpha 通道最大差 `0`；两版字块均 `2913` 像素 |
| 忽略区 | `30` 像素 —— 圆角外缘 alpha ≤ 31 的不可见毛边（8bit 预乘取整噪声，非几何差异） |
| `favicon.ico` 内含尺寸 | `16` / `32` / `48` |
| SVG 色值扫描 | `logo.svg` 仅 `currentColor`（hex 扫描为空）；静态变体色值只出现在 CSS 变量默认值中，可被 `--ba-logo-plate` / `--ba-logo-mark` 覆盖 |

自检明细（含 16 / 20px 字符栅格）由脚本输出到临时目录 `report.json`，重跑即可复现。

### 19.6 生成方式

```bash
# Windows（本仓库已验证环境；需 Pillow + 本机 Chrome 或 Edge）
C:\Users\Administrator\.workbuddy\binaries\python\envs\default\Scripts\python.exe assets/logo/build-assets.py
```

- 脚本是 `assets/logo/` 全部资源与各端 `apps/*/public/` 图标资产的**唯一生成入口**。
- 输出：`assets/logo/*`（含 `preview.png`）+ `apps/*/public/*`；同时打印验收自检与控制台字符栅格。
- 设计过程稿（三方案并排 / 精修前后对照 / 像素放大自检）：`.workbuddy/design/logo-drafts/compare.html`；其所需位图由同目录 `build-compare.py` 生成（`importlib` 复用 `render()`，避免两份栅格化实现漂移）。
