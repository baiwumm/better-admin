...
| 28 | 🚫 | 四端 `redirectToSignIn` 带参规则不统一 | `mechanisms.md:907`（已按现状改写） | ⚠️ **上轮我给的建议前提错误，先纠正**：不存在「一律带参的 React 基准形态」——**守卫层四端早已完全收敛**（React `routes/_authenticated/route.tsx:15-18` / Next `proxy.ts:36-47` / Vue `router/guards.ts:50-57` / Nuxt `middleware/auth.global.ts:49-56`，统一为「根路径裸跳、其余带 `redirect=完整路径`」），而 api-client 层 **React 恰是唯一从不带参的一端**。真实残留只在 **api-client 的 401 硬跳**：React `lib/api-client.ts:63-69`、Next `:37-42`、Vue `:77-84` 一律 `assign("/sign-in")`；Nuxt `app/lib/api-client.ts:84-97` 按守卫同规则。**两个收敛方向**：方案 B = 改三端向 Nuxt（3 处、各约 6 行）；**方案 C = 改 Nuxt 向其余三端（仅 1 处）**，且 C 更贴合「React 是基准」的既有裁决——代价是四端一致地放弃会话过期回跳（用户主动登出/未登录仍由守卫层带参，不受影响）。**我倾向 C**（改动最小、与基准端一致；会话过期属低频且落首页可接受）。附带：Nuxt `api-client.ts:80` 与 `:87` 重复赋值 `redirectingToSignIn = true`，任一方案都顺手删一处。消费侧 `isSafeRedirect` 四端均在，两方案都不触碰安全校验 |
