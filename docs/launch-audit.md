...
| 46 | ⬜ | 测试面真实缺口：`apps/next` / `apps/nest` 无 `test` 脚本（Next 0 测试文件）；website 无 lint/test/typecheck；Nest 测试基建备案仍成立 | 各 `package.json`；`docs/code-review-backlog.md:33-37` | 🚫 立项需拍板（引入框架属架构级） |
