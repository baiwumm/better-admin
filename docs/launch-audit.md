...
> **📦 已归档（2026-09-23）**：六端上线与收官事项全部完成后，本台账转为只读追溯用——后续不再更新条目（残余登记不修项已由 `docs/code-review-backlog.md` 与 progress.md 承接），仅供回查上线前夜的盘点取证。
>
...
| 55 | ✅ | 两份后端文档文首与 §8 仍是 Phase 2 立项口吻（「只做方案对齐、不编写 yaml」「建议结构 version: 1.0.0」），与其"已落地真源"身份不符；另 `apps/website` 无 `.env.example`（AGENTS §9 字面要求） | `apps/nest/docs/*.md` 文首 | **按建议执行：只加标头不逐句改**——两文件文首各加「文档状态：已落地并随实现持续更新（非立项提案）」，并写明真源归属（契约=openapi.yaml / 迁移=drizzle/ + schema 代码）与「立项口吻残留属历史记录、按 §13 不回改」。**website 那条我的结论是不补**：该端只读一个 `NEXT_MEM_CAP_MB` 构建开关、无任何密钥或后端变量，为凑 §9 造一份空模板是噪音而非价值；§9 的原意是防泄密与说明服务端变量，website 两者皆不涉及 |
...
| 51 | ✅ | 文档站 `theme-switch-animation` 仍锁 **0.1.0**，四端应用已升 0.2.0 | `apps/website/package.json`；`components/theme-toggle.tsx:36` 只用 `CIRCLE_BLUR`（0.2.0 移除的是 LTR/RTL/TTB/BTT 四向擦除，未被引用） | **已升 0.2.0**：`pnpm-workspace.yaml` 先补 `minimumReleaseAgeExclude` 条目（对齐四端同款注释），再 `pnpm add -E`；`theme-toggle.tsx` 无需改动（唯一引用的 `CIRCLE_BLUR` 在 0.2.0 仍存在）。验证：安装期 Lockfile 过供应链策略（379 条目）、`--frozen-lockfile` 通过、`next build` 成功 |
...
| 27 | ✅ | Nest 对 `phone` 不 trim、Next 会 trim（原记「待拍板」） | 取证：`apps/nest/src/account/dto/account.dto.ts:31-36` 只有 `@IsOptional` + `@Matches(/^1[3-9]\d{9}$/)`、**无 `@Transform`**；`main.ts` 全局 ValidationPipe 不做空白归一；`account.service.ts:147-148` 原样入库 | **已改**：Next `lib/server/account-service.ts:156-163` 去 trim；Nuxt `server/lib/account-service.ts:154-161` 同形问题同批改（只改 Next 等于把不一致换个对象）。**四端现状矩阵**：服务端 Nest / Next / Nuxt 三者已一致「不 trim、原样校验原样入库」；前端提交侧四端全部 trim（`react profile-form-card.tsx:75`、`next :77`、`vue ProfileFormCard.vue:50,77`、`nuxt :48,75`）⇒ 界面流量永远不带空白，本次改动只影响非 UI 客户端。**残留分歧已核清，我建议不改**：`phone: ""` Next 与 Nuxt 都把空串归一为 `null`（清空，见 `next app/api/account/profile/route.ts:43-49`、`nuxt server/api/account/profile.put.ts:24-29`），Nest 则 400——即分歧是「Nest vs 两个独立全栈端」，而 Next 与 Nuxt 之间是一致的；且「空串=清空」对 phone 合理、前端又永不下发空串，要对齐只能改 Nest（契约级行为变更、会牵动 React / Vue 错误提示路径），代价大于收益。**用户采纳：登记为已知良性差异、不改**——备案已写入 `docs/code-review-backlog.md`【暂缓 / 备案】段，含「将来若修应改 Nest 而非让两端学 400」的方向。本条结案 |
| 28 | ✅ | 四端 `redirectToSignIn` 带参规则不统一（真实分歧只在 api-client 层） | React `lib/api-client.ts:63-69`、Next `:37-42`、Vue `:77-84` 一律裸跳；Nuxt `app/lib/api-client.ts:84-97` 曾复刻守卫规则带参 | **用户拍板方案 C：Nuxt 改裸跳，向基准端收敛（只改 1 端）**。执行时保留 Nuxt 独有的「已在 `/sign-in` 则早退」防御（另三端缺这条，登录页收到意外 401 会整页重载），并顺手修掉一个真实缺陷——原 `redirectingToSignIn = true` 写在早退**之前**（:80 置位、:84 返回），会把标志位永久置真、反而挡掉之后的正常跳转，现移到早退之后。最终四端统一为「守卫层带参、api-client 层裸跳」。验证：nuxt eslint 0 error、vitest 10 文件 99 用例全绿、typecheck exit 0；mechanisms:907 备案同步改写 |
| 29 | ✅ | Next `/org/notices/[noticeId]` 详情页无 metadata（`mechanisms.md:735` 记为「待后续单独对齐」） | 复核：`apps/next/src/app/(authenticated)/org/notices/[noticeId]/page.tsx:6-7` **已有** `generateMetadata`，键 `menu.pageTitle.notices` 与 React `staticData.titleKey` 一致；`lib/route-title.ts:66-96` 已加 `[noticeId]` 模板匹配 | **前提已过期——其实早已实现**，无需改代码。真正做过的是：当年担心的「首帧正确→水合后退化」已由 route-title 模板匹配解除；另澄清两层口径（标签页标题走 metadata、具体公告标题走 tabs `syncMeta`，四端同构）。已把 mechanisms:735 那条过期表述按现状改写 |
...
- 第二批裁决：#18 Shadcn 条款**作废**（连带按实测重写 ui-spec §10.1 表单「现状」——原文整节是不存在的 shadcn Form 原语）；#47 `/settings` **四端统一不建页**（删三端页面 + 清两端登记表）；#55 两份后端文档加存档标头；#26 **不裁剪**双令牌并写进契约说明；#25 两级卡片分层**废弃**；#27 phone **Next + Nuxt 同批对齐 Nest**；#51 文档站依赖升 0.2.0；#29 复核为**早已实现**（前提过期）；Nuxt Dashboard 走查**已解决**、双端契约冒烟改**上线后全量跑**。

**⚠️ 本轮经复核推翻的自我纠错（防下轮误信）**
- #28：我上轮建议「统一为一律带参（React 基准形态）」**前提是错的**——守卫层四端早已收敛、React 恰是从不带参的一端，且"根路径不带参"是已拍板规则；真实残留仅 api-client 层三端裸跳。已改为待拍板方案 B。
- #27：用户只批了 Next，但 Nuxt 同形问题不改等于把不一致换个对象，故同批改了 Nuxt。
- #5：我上一轮称"已验证"却漏跑 `nuxt typecheck`，`alternate icon` 直接 TS2322；已按 React 基准改形并补验。
- #9 / #50 / #11 / #29 的原始台账描述各有数字或构成错误，均在对应行就地纠正。
