...
| 53 | ⬜ | `database-design.md` §2.1 `users` 字段表缺 2 列：`gender`（迁移 0008 引入）与 `token_version`（契约 v1.2 起 `integer notNull default 0`，是「改密码/封禁全端下线」的载体） | 复核自 #6 那轮；真源 `src/db/schema/users.schema.ts` + `drizzle/*.sql` | 补两列说明（token_version 的机制可引 mechanisms §5） |
| 54 | 🚫 | **§2 各表「类型」列整体失真**：文档写 `varchar(50)` / `varchar(255)`，实现全部是 `text` + DTO 长度校验（`grep -o varchar apps/nest/src/db/schema/*.ts` 0 命中） | 同上 | 属「文档改口径 vs schema 改回 varchar」的架构级选择，按 §18「不擅自修改架构」未动，需你拍板走哪条 |
