...
| 28 | 🚫 | 四端 `redirectToSignIn` 带参规则不统一 | `mechanisms.md:907` | ⚠️ **原备案前提已过期，且我给你的"按 React 基准统一为一律带参"是错的**：复核发现守卫层四端**早已收敛**（根裸跳 / 其余带参），React 恰是唯一从不带参的端，"一律带参"在代码里不存在；而"根路径不带参"是你 2026-09-14 已拍板规则（mechanisms §25）。真正残留的只有一处：**api-client 的 401 硬跳层**——React / Next / Vue 一律裸跳，Nuxt 按规则带参。**建议方案 B**：把这三端的 `redirectToSignIn` 平移成 Nuxt 形态（登录页早退 + 根路径裸跳 + 其余带参），消费侧 `isSafeRedirect` 四端都在、无需动安全校验；涉及 3 端 3 处，超出"小项"量级，等你点头再动。另发现 Nuxt `app/lib/api-client.ts:80` 与 `:87` 重复赋值 `redirectingToSignIn = true`，纯冗余可顺手删 |
