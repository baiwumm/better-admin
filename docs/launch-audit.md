...
## 处置进度（2026-09-22 本轮收尾）

**已修复并提交（一条一提交，累计 32 个提交；台账 38 个带状态行中 ✅ 26 项、🔧 半闭环 2 项）**：P0 #1 #2 #42 #43；P1 #6 #8 #11 #14 #15 #16 #19 #20 #21 #22 #23 #24 #50 #52 #53 #54；P2 #7 #13 #44 #48 #49。半闭环两项——#12（表述已改准，是否建 `docs/ → content/` 同步机制待拍板）、#47（死文件已删，`/settings` 四端统一形态待拍板）。
**逐项验证方式**：`nuxt build` / `nest build` / `tsc --noEmit`（react·next·nest）/ `next build`（文档站 20 页）/ `vitest run`（Vue 101 用例）/ `eslint` 逐文件 / `node --check` + 与 yaml 集合脚本比对 / 编译产物取值实测（JWT 配置五分支）/ `git check-ignore`（六端 env 规则）。

**仍开放（不属本轮「可自主修复」范围）**：
- 🚫 需你拍板：#18 Shadcn 幽灵条款、#25 两级卡片分层、#26 Next 响应体裁剪双令牌、#27 phone trim、#28 redirectToSignIn 统一、#29 Next 公告详情 metadata、#30 角色矩阵、#46 测试基建立项、#47 `/settings` 统一形态、#51 文档站依赖升级、#12 建否同步机制、**#45 挂 CI**、**#9 补超管授权（需写共用线上库）**。
- 📋 上线环节：#3 SPA 回退文件、#4 改默认密码、#5 五端品牌图标、#32–#39 走查与冒烟回收、`git tag v0.2.0`、`DEMO_MODE` 与 `LOG_API_SKIP_GET` 线上配置。
- ⏸️ P3 登记不修：#30 #39 两组已知差异；#31 #56 随下次变更处理。

**本轮两处自我纠正**（详见 #50 / #11 行与 progress 条目）：误判物理表 18 张（实为 17，`settings` 已 DROP）；误写文档站「Next 77 操作零缺口」（实为 76/77）。
