/**
 * 多标签页状态管理纯函数（无 Vue / store / 路由依赖；与 React 端 lib/tabs-model.ts 同构平移）。
 *
 * 从 tabs-store 抽离以便独立单测：
 * - 标签以 pathname 为键（与 KeepAliveOutlet 缓存键、菜单匹配、权限校验一致，
 *   search 变化不新开标签）；
 ...
 *   非目标项（保活实例上限另由 KeepAlive max 独立治理）。
 */
