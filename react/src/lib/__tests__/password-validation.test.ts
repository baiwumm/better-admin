...
/**
 * 密码策略（契约 v1.8.0）前端预检——用例与 Nest 端 password-policy 的边界口径一致，
 * 任一侧规则调整时两边用例须同步（前后端漂移由此暴露）。
 */
...
describe("getPasswordError · 用户名规则", () => {
...
describe("buildPasswordSchema", () => {
  it("issue.message 即 PasswordErrorKey，供表单拼 i18n key", () => {
    const schema = buildPasswordSchema("admin");
    const short = schema.safeParse("a1");
    const containing = schema.safeParse("admin12345");

    expect(short.success).toBe(false);
    expect(containing.success).toBe(false);
    if (!short.success) {
      expect(short.error.issues[0]?.message).toBe("tooShort");
    }
    if (!containing.success) {
      expect(containing.error.issues[0]?.message).toBe("containsUsername");
    }
    expect(schema.safeParse("Valid#123").success).toBe(true);
  });
});
