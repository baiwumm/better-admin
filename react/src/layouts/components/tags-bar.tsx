 ...
 *   排序激活后经 sortMovedRef 吞掉随后的 click / press，防止误切换标签
 *   （onPress 先于 onDragEnd 触发，故 ref 在 onDragStart 置位、rAF 兜底复位）；
 * - 右键菜单、中键关闭不受影响（PointerSensor 仅响应左键）；
 ...
 *   「关闭标签 = 销毁对应保活实例」；排序仅改展示序，保活池按插入序
 *   常驻（reconcileWithTabs 按 Set 内容幂等），实例不因排序移动 DOM。
 */
          ...
          >
            <ul className="flex w-max items-center gap-1 px-0.5 py-1.5">
              {paths.map((path) => {
                ...
                const routeStatic = findRouteStaticMeta(
                  routeStaticByPath,
                  path,
                );
                const title =
 ...
 * - 拖拽中 z-10 置顶 + 阴影（ul 为 flex 容器，z-index 直接生效）；
