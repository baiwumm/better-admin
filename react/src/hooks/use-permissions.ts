...
function useCurrentMenuPermissions(): string | null {
  ...
  return useMemo(() => {
    ...
    const leafMenus = flattenLeafMenus(menuTree);
...
export function useMenuPermissions() {
  ...
  const bits = BigInt(menuPermissions);

  return {
    canSearch: hasPermission(bits, bitsMap.get("SEARCH") ?? 0n),
    canAdd: hasPermission(bits, bitsMap.get("ADD") ?? 0n),
    canEdit: hasPermission(bits, bitsMap.get("EDIT") ?? 0n),
    canDelete: hasPermission(bits, bitsMap.get("DELETE") ?? 0n),
    canBatchDelete: hasPermission(bits, bitsMap.get("BATCH_DELETE") ?? 0n),
    canAddChild: hasPermission(bits, bitsMap.get("ADD_CHILD") ?? 0n),
