...
export function NoticeDetailPage() {
  ...
  // strict: false：组件被 KeepAlive 实例池保活（hidden）时路由已切往他页，
  // 严格 from 匹配会因找不到活跃 match 抛 Invariant 使整个布局崩溃；
  // 宽松模式返回全部 params，hidden 期间实例不发请求，无副作用。
  const { noticeId } = useParams({ strict: false }) as { noticeId: string };
        ...
        <Card.Content className="flex flex-col gap-5 p-6 md:p-8">
          ...
          <div>
            <Button
              ...
              variant="ghost"
              onPress={() =>
                void navigate({ to: canAccessList ? "/org/notices" : "/" })
              }
            >
