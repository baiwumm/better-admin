...
/**
 * 全局前置守卫三层（平移自 vue/src/router/guards.ts，语义对齐 React 端
 * beforeLoad + useMenuRouteGuard）：
 *
 ...
 *
 * 权威仍是 API 层鉴权（Nitro server 端 requireAuthUser）；
 * 路由切换进度条由 NuxtLoadingIndicator 承担（nuxt-plan §4 双轨拍板），
 * 不再在此触发路由进度。
 */
