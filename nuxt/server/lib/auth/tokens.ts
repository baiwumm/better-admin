...
/**
 * JWT 双令牌签发/校验（jose，逐字平移自 next/src/lib/server/auth/tokens.ts，
 * 仅移除 "server-only" 导入；与 Nest 端 jsonwebtoken 语义对齐）。
 *
 * 与 Nest 端的差异仅在实现库：
 * - Nest 用 passport-jwt + @nestjs/jwt；Next / Nuxt 用 jose（同为 HS256）；
