...
import { batchRemoveLogs } from '../lib/logs-service'
import { jsonOk, handleRouteError } from '../lib/route-helpers'

/**
 * DELETE /api/logs?ids=（BATCH_DELETE 位）：批量删除（query ids 逗号分隔，全有全无）。
 */
export default defineEventHandler(async (event) => {
  try {
    ...
    return handleRouteError(event, error, { path: '/api/logs', method: 'DELETE' })
  }
})
