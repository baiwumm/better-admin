...
**实现硬性约束（禁止 N+1 查询）**：服务端严禁对每个菜单节点单独执行 `role_menus` 查询来填充 `userPermissions`。必须遵循以下 **O(1) 内存映射法**，将数据库查询控制在常数次：

1. 一次性查询当前用户所有 `role_menus` 记录（`SELECT menu_id, permissions FROM role_menus WHERE role_id IN (...)`），组装为 `Map<menuId, permissions>`（JavaScript 中为 `Map<string, bigint>`）。
2. 递归/遍历菜单树时，对每个节点直接通过 `map.get(menu.id)` 获取权限位，若不存在则默认为 `0`。

此方式确保无论菜单树有多少节点（如 100+ 或 500+），数据库查询次数恒定（仅 2 次：查菜单树 + 查权限映射），彻底避免 N+1 性能瓶颈。若后续引入 Redis 缓存，亦需保持此「批量预加载」逻辑不变，不得退化为逐节点查询。
...
| target | varchar(10) | NOT NULL, DEFAULT '_self' | 打开方式 `_self`/`_blank` |
