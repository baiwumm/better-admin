...
- **当前待办**：**Nuxt 端 M5 剩余项（部署）**（Vercel 部署 nuxt.baiwumm.com（Nitro node preset）+ 线上冒烟 + git tag v0.2.0——**随四端统一上线执行**，文档收尾部分已于 2026-09-13 完成）；**Playground 演示场（先行，2026-09-14 拍板；Phase A 同日完成）** + 演示上线准备 + Dashboard 概览页（计划清单见 [`docs/plan-dashboard-playground.md`](docs/plan-dashboard-playground.md)：**Phase A/B Playground 不等 Gate**——纯前端静态演示、四端并发，菜单已定稿 rare-ui 组件集 + number-flow；**Phase A 已完成并验收**（菜单树 10 节点含三级经 `apps/nest/scripts/migrate-menus-add-playground.ts` 录入共用库、全部 0 位；服务端菜单可见性同日修正为「有 role_menus 关联记录即可见」（Nest / Next / Nuxt 三端同款移除 `!= 0` 过滤，`database-design.md` §1.5 v0.10）；四端占位页 + i18n + 守卫登记，四端检查通过；**React 端 GUI 冒烟通过（2026-09-15）**：角色授权可见性 / 三级菜单 / 占位页正常，其余三端 GUI 冒烟随 Phase B 逐端对齐时执行）；**Phase B React 基准已完成（2026-09-15，用户 GUI 验证通过）**——`PlaygroundIntro` + `meta.ts` / `registry.ts` 机制（版本自 package.json 编译期收敛读取）+ rare-ui 六组件 vendor（`cn` 改 `@heroui/react`、token 替换、github-activity 加 i18n 出口）+ `@number-flow/react` 共 7 页落地，依赖 `motion` / `prism-react-renderer` / `@number-flow/react` 锁版引入，i18n 116 键 React + Next 语言包同步；**⚠️ 引入会自行输出 className 的第三方渲染器时须先查与 HeroUI 全局 BEM 类撞名**（`mechanisms.md` §27，本次 prism `.tag` 撞 HeroUI Tag 样式）；**Phase B Next 对齐已完成（2026-09-15）**——`features/playground/` 自 React 端整体平移（客户端 tsx 加回 `"use client"`、meta `source` 指向 `apps/next/`、`usedIn` 跳转改 `next/navigation` `useRouter`），page.tsx 保持服务端组件导出 `generateRouteMetadata`，三依赖同版锁入，lint 0 error / tsc 零错误 / build / check-locales 通过（GUI 冒烟待用户本地验证，详见 progress.md 置顶条目）；**Phase B Vue / Nuxt 对齐已完成（2026-09-16，定时任务无人值守执行）**——按 React 视觉以 Nuxt UI 内置组件 + 自定义组件重写 7 页与共享机制，依赖锁版 `clsx` + `tailwind-merge`（新增 `cn`）/ `@number-flow/vue` / `prismjs`（`prism-react-renderer` 为 React 专属，Vue 侧 prism 直渲染），动画全部 CSS / TransitionGroup / Canvas 等效、**motion-v 评审结论不引入**，`useDemoActive` 统一 keepAlive 暂停；两端 lint / test 全绿 + GUI 冒烟通过，**用户本地 GUI 验证通过（2026-09-16，顺带修复 Grid Reveal 说明条 out-in 高频切换不可见——`mechanisms.md` §28：motion popLayout 的 Vue 等效写法是并行进出 + 离场 absolute）**，Nuxt typecheck / build 仅受上述 logs 缺失阻塞；**Playground 四端全部对齐，步骤 4 销项**（遗留：React 语言包 `codeBlock.inlineLead` / `numberFlow.description` 文案写死「React 端 / @number-flow/react」，经 sync-locales 同步到 Vue / Nuxt 显示为 React 措辞，待用户拍板改为技术栈中立文案；详见 progress.md 置顶条目）；**Playground 追加主题切换动画页四端对齐已完成（2026-09-16）**——`theme-switch-animation@0.1.0` 四端引入（React `db5f48a` / Next `4935ce9` / Vue `6a17381` / Nuxt `dda2ad6`），第 8 个演示页（菜单 `sun-moon` sort 4 排 GitHub Activity 后，幂等脚本已录入共用库）；store `applyThemeModeInstant` 让渡 VT 编排权 + `data-theme-demo-vt` 摘名规则见 progress.md 置顶条目与 `mechanisms.md` §29，GUI 待用户本地验证；**Gate（Nuxt 端功能全部完成 + 冒烟测试通过）后**按 Phase 0 演示上线准备（faker 数据重置 / 只读守卫 / 快捷登录 / 日志降噪）→ Phase C Dashboard 推进；图表库已定 Recharts）；CI 挂接（四端 lint / test / build，现有 workflows 仅 check-locales / clean-logs 两个 cron）；**四端统一上线**（§17 上线状态标记：React / Vue Cloudflare Pages + Next / Nuxt / website Vercel + Nest Render，Vue 线上冒烟清单见 `docs/vue-plan.md` §M4，均为部署环节执行）。Vue / Nuxt 后续实现时直接跟上最新契约版本（详见 progress.md 各阶段条目）。
