...
- **Playground 状态快照（2026-09-16 全部完成并验收）**：Phase A（菜单树 10 节点含项目首个三级菜单 + 四端占位；服务端菜单可见性同日修正为「有 role_menus 关联记录即可见」，`database-design.md` §1.5 v0.10）+ Phase B（React 基准 8 页 → Next 平移 / Vue·Nuxt 按 Nuxt UI 重写，含追加的主题切换动画页）四端全部对齐；计划 §9.1 验收清单五项经用户 GUI 实测逐项通过（含折叠态三级菜单 / 面包屑 / 命令面板搜索；语言包措辞已改技术栈中立 `74db037`，logs 模块 P0 已修复 `60546ab`）。实施细节见 progress.md 2026-09-14 ~ 09-16 条目；**⚠️ 引入会自行输出 className 的第三方渲染器时须先查与 HeroUI 全局 BEM 类撞名**（`mechanisms.md` §27，本次 prism `.tag` 撞 HeroUI Tag 样式）。
