...
- **当前待办**：**Phase 0 演示上线准备**（Gate-1 / Gate-2 已于 2026-09-16 达成，执行清单见 [`docs/plan-dashboard-playground.md`](docs/plan-dashboard-playground.md) §3.6：OpenAPI v1.10.0 → faker 重置脚本 → Nest 只读守卫与 demo-login → 四端快捷登录与 DEMO_READONLY toast → 统一验收上线）；**Phase C Dashboard**（**等 Phase 0 完成后启动**，faker 数据集为数据底座）；**Nuxt 端 M5 剩余项（部署）**（Vercel 部署 nuxt.baiwumm.com（Nitro node preset）+ 线上冒烟 + git tag v0.2.0——**随四端统一上线执行**）；CI 挂接（四端 lint / test / build，现有 workflows 仅 check-locales / clean-logs 两个 cron）；**四端统一上线**（§17 上线状态标记：React / Vue Cloudflare Pages + Next / Nuxt / website Vercel + Nest Render，Vue 线上冒烟清单见 `docs/vue-plan.md` §M4，均为部署环节执行）。Vue / Nuxt 后续实现时直接跟上最新契约版本（详见 progress.md 各阶段条目）。
