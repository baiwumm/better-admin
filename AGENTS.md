...
- **当前阶段**：React 端业务模块迁移推进中（React + NestJS 全栈阶段）。已完成：React Hero UI 迁移启动、NestJS + PostgreSQL 后端（契约 v1.4.5），React 端登录认证 / 全站国际化 / 权限管理 / 菜单管理 / 字典管理 / 角色管理 / 用户管理均已上线。
- **当前待办（记录在案）**：日志管理、概览 Dashboard 仍待迁移；`roles.enabled` 参与权限聚合待评估；存量库需执行 `nest/scripts/migrate-menus-add-grant-bit.ts` 补录角色管理菜单 GRANT 声明位（v1.4.4）。
