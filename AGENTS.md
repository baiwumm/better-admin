...
- **当前待办**：**角色关联用户列（契约 v1.13.0，2026-09-20）**：Nest + React 已完成并冒烟通过（列表 `userCount`/`readers` + `GET /roles/{id}/users` 名单穿透抽屉），**Next / Vue / Nuxt 已全部对齐（2026-09-21，四绿 + 冒烟通过，Nuxt 收尾后功能四端 100%），各端 GUI 走查待用户**，见 progress.md 置顶条目。**Phase 0 演示上线准备已完成（2026-09-18）**——契约 v1.10.0 + Nest 只读守卫与 demo-login（T1）、faker 重置脚本与幂等验证（T2）、四端快捷登录与 DEMO_READONLY toast（T3/T4）、全链路验收与文档收尾（T5）全部完成，验收报告见 [`docs/plan-phase0-execution.md`](docs/plan-phase0-execution.md) §4；**线上 `DEMO_MODE=true` 配置与 §5 人工清单随四端统一上线执行**。下一步：**Dashboard 余端对齐**——React 基准已完成（2026-09-19 二轮打磨，契约升到 **v1.12.0**：`stats/overview` 移除 `days` 参数、`loginTrend` 固定近 30 日、7/30 区间由前端本地截取）；Next 端已同步 v1.12.0 取数形状与本基准全部打磨项（2026-09-19，像素级观感待用户 GUI 走查）；Vue 端**已按本基准完成对齐（2026-09-19，改动已入库，待用户 GUI 审核）**：控件全用 Nuxt UI 内置，图表按 §21 三级评审引入 **Unovis 1.7.0**（`@unovis/vue` + `@unovis/ts` 精确锁版；2026-09-20 由早期「零依赖手写内联 SVG」迁移而来，选型与实测见 mechanisms §32，补记见 progress.md 2026-09-22 条目），token 换挂 `--ui-*`（详见 progress.md 置顶条目）；**同日代码审查修复两处平移缺陷**（KpiCard 渐变 id 的 `<script setup>` 实例级撞 id → Vue 3.5 `useId()`、环形图无用 `<circle>` 残留）并**销掉 format-date「天」档 `24*24` 换算备案**（Vue / Nuxt 各一行，见 progress.md）；**Nuxt 端 Dashboard 已完成（2026-09-19，改动已入库，待 GUI 走查 + 契约冒烟）**：图表按用户拍板引入 `nuxt-charts` 3.0.0（Vue 端另按 §21 评审选 **Unovis**，两端不强求同源引擎，评审结论见 mechanisms §32），其余见 §19 当前阶段 M-Dashboard 段；两项设计决策待用户拍板（~~角色环形图单色透明度阶梯相邻难分辨~~ **已解决（2026-09-19）**：改为 `--accent` 色相轮转分类色，恒定 L/C，见 progress.md 同日条目；仅剩**两级卡片分层**待拍板）；**Nuxt 端 M5 剩余项（部署）**（Vercel 部署 nuxt.baiwumm.com（Nitro node preset）+ 线上冒烟 + git tag v0.2.0——**随四端统一上线执行**）；~~CI 挂接~~ **已完成（2026-09-22）**：`.github/workflows/ci.yml` 六端 17 步矩阵（lint / typecheck / test / build，pnpm 版本按各端自身声明、node 22 与既有 workflow 对齐）；**四端统一上线**（§17 上线状态标记：React / Vue Cloudflare Workers（Static Assets）+ Next / Nuxt / website Vercel + Nest Render，Vue 线上冒烟清单见 `docs/vue-plan.md` §M4，均为部署环节执行）。Vue / Nuxt 后续实现时直接跟上最新契约版本（详见 progress.md 各阶段条目）。
