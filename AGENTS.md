...
- **当前待办**：**Vue 端 Vercel 部署 + 线上冒烟（用户手动执行，清单见 `docs/vue-plan.md` §M4）**；**Nuxt 端计划评审**（`docs/nuxt-plan.md` §5 六项决策待拍板）通过后启动 M0；演示上线准备 + Dashboard 概览页 + Playground 演示场（计划清单见 [`docs/plan-dashboard-playground.md`](docs/plan-dashboard-playground.md)：整体排在 Vue 全功能对齐 + 冒烟测试通过之后，顺序 Phase 0 演示上线准备（faker 数据重置 / 只读守卫 / 快捷登录 / 日志降噪）→ Phase C Dashboard → Phase A/B Playground；图表库已定 Recharts）；Next.js Vercel 部署 + CI 挂接；`docs/mechanisms.md` 沉淀 Next 期机制结论。Vue / Nuxt 后续实现时直接跟上最新契约版本（详见 progress.md 各阶段条目）。
