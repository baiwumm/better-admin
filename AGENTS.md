...
- **Playground 状态快照（2026-09-16 全部完成并验收）**：Phase A（菜单树 10 节点含项目首个三级菜单 + 四端占位；服务端菜单可见性同日修正为「有 role_menus 关联记录即可见」，`database-design.md` §1.5 v0.10）+ Phase B（React 基准 8 页 → Next 平移 / Vue·Nuxt 按 Nuxt UI 重写，含追加的主题切换动画页）四端全部对齐；**当前态（2026-09-21）：第 9 页「加载动画」（`/playground/loaders`，beUI `loader` 17 变体）四端 100% 对齐**——React 基准 `d12f190` → Next `37be50a` → Vue `a8899d9` → Nuxt（自 Vue 平移）；Vue / Nuxt 两端按 §21 不引 motion-v，motion 关键帧一律 CSS `@keyframes` 等效（morph 走 `clip-path: polygon()`），Nuxt 端因 SSR 把 reduced-motion 的 metaballs 合拢姿态改由 CSS 媒体查询达成；另：Vue / Nuxt 端 `theme-switch-animation` 页此前漏登记菜单权限表，已按 mechanisms §34 补登记；**第 10 页「组织架构树」（`/playground/okr-tree`，`react-okr-tree@1.13.0`，外观经 `--okr-*` 变量挂站点 token 随暗色联动、不引 `html-to-image`）React 基准已完成（lint / tsc / build / test / check-locales 全绿 + 菜单已入共用库），待用户 GUI 验证后按序对齐 Next / Vue / Nuxt**（Vue / Nuxt 端装 `vue3-okr-tree`，§21 评审已由用户批准；语言包已随 React 同步 Next）**；计划 §9.1 验收清单五项经用户 GUI 实测逐项通过（含折叠态三级菜单 / 面包屑 / 命令面板搜索；语言包措辞已改技术栈中立 `74db037`，logs 模块 P0 已修复 `60546ab`）。实施细节见 progress.md 2026-09-14 ~ 09-16 条目；**⚠️ 引入会自行输出 className 的第三方渲染器时须先查与 HeroUI 全局 BEM 类撞名**（`mechanisms.md` §27，本次 prism `.tag` 撞 HeroUI Tag 样式）。
