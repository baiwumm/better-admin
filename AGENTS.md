...
- **当前待办**：① **refresh token JWT 同秒重复缺陷待拍板**（2026-09-25 Nest e2e 落地时发现）——refresh token payload 无 `jti` 且 `iat` 秒级精度，同一用户同秒二次登录签发完全相同令牌串，撞 `refresh_tokens.token_hash` 全局唯一索引 → 500 INTERNAL_ERROR；根治方向：payload 加 `jti`（nanoid）或唯一索引放宽为 `(user_id, token_hash)`，属认证方案变更待用户拍板，机理见 `docs/mechanisms.md` §42 坑 4。② CI `nest-e2e` job 需在仓库 Settings > Secrets 配置 `TEST_DATABASE_URL`（Supabase 5432 直连串），未配置前该 job 会红。其余已清零（2026-09-23 收官）；新待办出现时在此登记。
