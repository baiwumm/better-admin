...
/**
 * 通讯录 Excel 导出（write-excel-file，对齐 React 端 directory-export.ts）。
 *
 ...
 * - 上限第一版按演示规模设 10000 条；现阶段无后端导出接口（无契约变更）。
 *
 * 表格样式（企业级，与 React 端逐项一致）：
 ...
 * - 4.x 的 writeXlsxFile 不消费 columns.header/cell，必须先经库导出的
 *   getSheetData 把 objects 转成 SheetData（自动拼表头行 + 应用单元格样式）。
 */

/** vue-i18n 翻译函数最小接口（避免耦合 ComposerTranslation 泛型） */
 ...
 */
function buildColumns(t: Translate, employedLabel: string): Column<string[]>[] {
...
export async function exportDirectoryExcel(options: {
  ...
  // getSheetData 把 objects 转成 SheetData，否则表头行不会生成、所有样式丢失
