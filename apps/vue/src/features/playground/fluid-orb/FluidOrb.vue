...
/**
 * 流体光球（按 rare-ui `fluid-orb` 移植，MIT）：原生 WebGL 片元着色器 fbm 噪声，零 npm 依赖。
 * 着色器程序与 RAF 循环在 `watch([active, size, color])` 内建立、cleanup 释放：
 * keepAlive 切走即暂停（GL 资源释放），切回重建；参数变更重建程序。
 */
const props = withDefaults(
  defineProps<{ size?: number; color?: string; class?: string }>(),
  { size: 240, color: "#1A73F2", class: undefined },
);
...
watch(
  [active, () => props.size, () => props.color, canvasRef],
  ([on, size, color, canvas], _prev, onCleanup) => {
    if (!on || !canvas) return;
...
<template>
  <div
    :class="cn('relative overflow-hidden rounded-full', props.class)"
