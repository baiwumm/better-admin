...
/**
 * 演示场 › 组织架构树（vue3-okr-tree：组织架构 + 飞书 OKR 双向展开）。
 *
 * 与 React 基准对齐：数据本地工厂取数（命令式方法回写不互相污染）、
 * 外观经 --okr-* 变量挂 Nuxt UI token（okr-tree.css）、不引入可选 peer
 * html-to-image（导出图能力不在演示范围）。默认方向纵向（与 React / Next 一致）。
...
function fit() {
  ...
  outerRaf = requestAnimationFrame(() => {
    innerRaf = requestAnimationFrame(() => {
      const viewport =
        canvasRef.value?.querySelector<HTMLElement>(".okr-viewport");
...
// （immediate 与 React 端挂载即调用对齐；fit 内部有视口未挂载守卫）
watch(
  keyword,
  (value) => {
  ...
  <PlaygroundPage :meta="okrTreeMeta">
      ...
      <template #controls>
        ...
        <DemoControl :label="t('features.playground.okrTree.expand')">
          <div class="flex items-center gap-2">
            <UButton size="sm" variant="outline" @click="handleExpandAll">
              {{ t("features.playground.okrTree.expandAll") }}
            </UButton>
            <UButton size="sm" variant="outline" @click="handleCollapseAll">
      ...
      >
        ...
        >
          <template #toolbar="scope">
            <ViewportToolbar :scope="scope" @reset="fit" />
          </template>
          <!--
          ...
          >
            <template #default="{ node, data }">
              <OrgNodeCard :node="node" :data="data as OrgNodeData" />
            </template>
            <template #empty>
    ...
    >
      ...
      <div class="okr-tree-demo overflow-x-auto rounded-2xl bg-elevated/40 p-4">
        ...
        >
          <template #default="{ node, data }">
            <OkrNodeCard :node="node" :data="data as OkrNodeData" />
          </template>
        </VueOkrTree>
      </div>
    </DemoSection>
