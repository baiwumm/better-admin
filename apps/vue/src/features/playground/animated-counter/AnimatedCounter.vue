...
<template>
  ...
  >
    <span v-if="prefix != null || $slots.prefix" class="inline-block">
      <slot name="prefix">{{ prefix }}</slot>
    </span>

    <span class="sr-only">{{ negative ? "-" : "" }}{{ chars }}</span>

    <span aria-hidden="true" class="inline-flex select-none items-center">
      <span v-if="negative" class="inline-block">-</span>
      <TransitionGroup
      ...
      >
        <template v-for="cell in cells" :key="cell.key">
          <AnimatedCounterDigit
            ...
            :reduced="reduced"
          />
          <span v-else class="inline-block" data-slot="animated-counter-mark">{{
            cell.char
          }}</span>
        </template>
      </TransitionGroup>
    ...
    <span v-if="suffix != null || $slots.suffix" class="inline-block">
