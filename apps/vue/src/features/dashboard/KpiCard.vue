...
import { areaPath, relativePoints, smoothPath } from "./chart-geometry";

/**
 * KPI 统计卡（对齐 React 端 kpi-card）：卡头 = 左上图标徽标 + 标题 + 右侧状态
 * badge；卡体 = NumberFlow 大数字 + 底部通栏迷你 sparkline。
 *
 * sparkline 用 `preserveAspectRatio="none"` 铺满卡宽，因此描边必须带
 * `vector-effect="non-scaling-stroke"`——否则非等比缩放会让线宽随方向变化
 * （横段粗、竖段细），曲线也被压平看不出起伏。
 * 颜色一律走内联 style 的 CSS 变量：SVG 呈现属性（fill / stroke / stop-color）
 * 在部分引擎里不解析 var()，style 属性才是可靠通道。
 * 无 series 的卡（组织规模）不留空带，数字顶部对齐、底部留白（同 React 口径：
 * 同排四卡的大数字必须共用基线，错位比留白更显眼）。
 */
...
/**
 * 渐变 id（SVG defs 全局作用域，避免多卡同 id 冲突）。
 * 注意不能用 React 端「模块级自增序号」的写法：`<script setup>` 顶层代码
 * 每个实例都会重新执行，自增序号在四张卡上永远得到同一个值；改用 Vue 3.5
 * 内置 useId()，每实例唯一。
 */
...
const props = withDefaults(
  ...
  { badge: undefined, badgeTone: "neutral", series: undefined },
);
...
<template>
  ...
  >
    ...
    >
      ...
      <path :d="spark.area" :style="`fill: url(#${gradientId})`" />
      <path
        :d="spark.line"
