 ...
 *
 * 职责：
 * - 自动附带 `Authorization: Bearer <accessToken>`（从 auth-store 注入）。
 ...
 * - refresh 失败（refreshToken 也过期）：清空本地会话并整页跳转登录页。
 *
 * 注意：本文件不直接 import auth-store（避免循环依赖），
 * 仅通过 `bindAuthSnapshot()` 注入的最小读写接口访问 token。
 */
...
/** 由 auth-store 初始化时注入，供 api-client 读写 token（main.ts 启动时调用一次）。 */
...
/** 全局错误监听（App 层注册，用于把 5xx 等错误桥接到 toast；见 AppShell.vue）。 */
...
function notifyErrorHandler(error: ApiClientError) {
...
export async function fetchApiRaw(
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const apiError = new ApiClientError(response.status, err.code, message);
