...
/**
 * 最近动态卡（对齐 React / Vue 端 recent-activity-card）：用户首字头像 +
 * 操作人 + 动作人话文案 + 相对时间。数据为 type=operation 日志（服务端已剔除
 * 敏感字段；接口不返回头像 URL 时以姓名首字 fallback 呈现）。
 */
      ...
      <template v-else>
        ...
        >
          ...
          <p class="min-w-0 flex-1 truncate text-sm">
            ...
            <span
              v-else
              class="text-muted ml-1.5 font-mono text-xs"
            >
              {{ item.action }}
            </span>
          </p>
