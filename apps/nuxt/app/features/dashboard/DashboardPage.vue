...
/**
 * Dashboard 概览页（对齐 React 基准 dashboard-page，契约 v1.12.0；与 Vue 端
 * 同构，两处图表按用户拍板改用 nuxt-charts，其余 Nuxt UI 组件与 Vue 端同款）。
 *
 ...
 *   切 Tabs 不发请求；
 * - 图表：登录趋势 AreaChart / 角色占比 DonutChart（nuxt-charts v3，锁
 *   3.0.0），KPI sparkline 仍手写 SVG（与 React / Vue 端同一口径）；
...
/**
 * 挂载门闩（与 Next 端 mounted / SSR 恒输出骨架同一口径）：
 * SSR 期既不执行 stats 聚合查询，也不渲染 nuxt-charts 图表（其依赖树
 * motion-v / vccs 在服务端渲染会挂起 SSR 流），客户端挂载后才启用查询。
 */
 ...
 * 最近动态取 7 条、公告 5 条（服务端固定返回口径）：动态单行约 50px、
 * 公告两行约 68px，两卡内容高度基本齐平，并排时不出现整块底部空白。
 */
