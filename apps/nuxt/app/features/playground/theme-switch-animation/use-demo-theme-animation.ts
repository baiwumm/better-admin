...
/** 演示页在转场期间挂到 `<html>` 的「摘名属性」（规则见 assets/css/theme-transition.css）。 */
...
/**
 * 演示用主题切换：受控模式接入项目主题 store（对齐 React / Vue 端 `useDemoThemeAnimation`）。
 *
 * `useThemeAnimation` 由 `theme-switch-animation/nuxt` 模块自动导入（无需 import）。
 *
 ...
 */
export function useDemoThemeAnimation<T extends HTMLElement = HTMLButtonElement>(
