import type { ListQueryParams, Log } from '@/lib/api-types'
...
/** 日志操作人摘要（无关联用户返回 null，显示占位符） */
export function logOperator(log: Log): {
  username: string
  displayName: string
  email: string
  avatar: string | null
  ...
  return {
    ...
    // 关联用户存在时 username 恒非空；displayName/email 允许为空，回退 username
    displayName: log.displayName ?? log.username,
    email: log.email ?? log.username,
    avatar: log.avatar
  }
}
...
export function getLogErrorMessage(error: unknown): string {
  ...
  switch (code) {
    ...
    case 'INVALID_OPERATION':
      return getErrorMessage('errors.logs.invalidOperation', '部分日志 ID 无效')
    default:
