 ...
 * 本地改动：`cn` 改自 @heroui/react 导入；Next 端在文件顶部加回 "use client"；
  ...
  return (
    ...
    >
      ...
      {!(showFrame && showHeader) && showCopyButton && (
        <CopyButton floating code={trimmed} />
      )}
      ...
      <div
        ...
        role="region"
        // 可滚动区域需可聚焦以支持键盘滚动（WCAG 2.1.1），是该规则文档列明的合理例外
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="pr-3.5">
                        {line.map((token, key) => {
                          // 只取 style / children、不透传 prism 的语义 className（如 "token tag"）：
                          // HeroUI 全局 BEM 类 .tag 会把标签 token 渲染成 inline-flex，令属性间的
                          // 空白 token 折叠为零宽；高亮颜色本就走内联 style，丢弃 className 无损。
                          const { style: tokenStyle, children } = getTokenProps(
                            {
                              token,
                            },
                          );
