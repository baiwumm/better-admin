...
/**
 * Dashboard 概览统计（契约 v1.11.0，只读聚合）。
 *
...
/** 日期序列起点（首日 UTC+8 0 点对应的 UTC 时刻） */
function seriesSince(dates: string[]): Date {
  return new Date(Date.parse(`${dates[0]}T00:00:00Z`) - UTC8_OFFSET_MS);
}
...
export class StatsService {
  /** GET /stats/overview — Dashboard 概览（一次返回全量区块数据） */
  async overview(days: 7 | 30) {
