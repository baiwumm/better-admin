...
/**
 * 幂等迁移：演示场补录「加载动画」子菜单（playground/loaders）。
 *
 * 背景：菜单是四端共用的单点数据源。演示场目录与既有页面已由
 * migrate-menus-add-playground.ts / migrate-menus-add-theme-switch-animation.ts
 * 录入；本页是后续追加项，故按仓库惯例单独建脚本（migrate-menus-add-<feature>.ts），
 * 不重跑已执行过的脚本。
 *
 * 行为（重复执行结果不变，幂等）：
 * 1. 按 i18nKey='menu.playground.loaders' 查重，不存在则插入到
 *    「演示场」（menu.playground）之下，sort=5 排在「主题切换动画」之后；
 ...
 *
 * 执行：pnpm tsx scripts/migrate-menus-add-playground-loaders.ts
 */

const TAG = '[migrate-menus-add-playground-loaders]';
...
async function main() {
  ...
  // 2. 子菜单：加载动画（页面节点：keepAlive 开启；纯展示页 permissions = 0）
  const [existing] = await db
    .select({ id: menus.id })
    .from(menus)
    .where(
      and(
        eq(menus.i18nKey, 'menu.playground.loaders'),
  ...
  let menuId: string;
  if (existing) {
    console.log(`${TAG} 子菜单「加载动画」已存在(id=${existing.id})，跳过插入。`);
    menuId = existing.id;
  } else {
    const [created] = await db
      ...
      .values({
        id: nanoid(),
        label: '加载动画',
        i18nKey: 'menu.playground.loaders',
        icon: 'loader-circle',
        to: '/playground/loaders',
    ...
    menuId = created.id;
    console.log(
      `${TAG} 已插入页面「加载动画」(id=${menuId}, to=/playground/loaders)。`,
    );
  }
